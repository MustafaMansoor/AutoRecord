...
const getAllCompanyPeople = async (req, res) => {
  try {
    ...
    if (userRole === 'admin') {
      companies = await Company.find({ admin: userId }).populate('people', 'username email');
    } else {
      companies = await Company.find({ people: userId }).populate('people', 'username email');
    }
    ...
    console.error("Error fetching companies and people:", error);
    res.status(500).json({ error: "Internal Server Error" });
  }
};
