...
import "./EditSaleImageDetail.css";
...
function EditSaleImageDetail({ data }) {
  ...
  async function handleSubmit(status, reason = "") {
    ...
    const saleId = formData._id;

    try {
      const response = await fetch(
        `http://localhost:3000/api/companies/${companyId}/sales/${saleId}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="edit-page-main-container">
      ...
      <Dialog open={dialogOpen} onClose={handleDialogClose}>
        <DialogTitle>Please Enter Reason for Rejection</DialogTitle>
        <DialogContent>
          <DialogContentText>
            To reject this sale, please provide a reason for the rejection.
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
