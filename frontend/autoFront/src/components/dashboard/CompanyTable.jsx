        ...
        companies.map(async (company) => {
          try {
            const [purchases, sales, suppliers] = await Promise.all([
              axios.get(
                `http://localhost:3000/api/companies/${company._id}/purchases`
              ),
              axios.get(
                `http://localhost:3000/api/companies/${company._id}/sales`
              ),
              axios.get(
                `http://localhost:3000/api/companies/${company._id}/suppliers`
              ),
            ]);

            // Log the responses to check the structure
            console.log(
              `Purchases for company ${company._id}:`,
              purchases.data
            );
            console.log(`Sales for company ${company._id}:`, sales.data);
            console.log(
              `Suppliers for company ${company._id}:`,
              suppliers.data
            );

            return {
              ...company,
              purchasesCount: purchases.data.purchases
                ? purchases.data.purchases.length
                : 0,
              salesCount: sales.data.sales ? sales.data.sales.length : 0,
              suppliersCount: suppliers.data.suppliers
                ? suppliers.data.suppliers.length
                : 0,
            };
          } catch (error) {
            console.error(
              `Error fetching details for company ${company._id}:`,
              error
            );
            return {
              ...company,
        ...
        <TableBody>
          {companyData.map((company) => (
            <TableRow key={company._id}>
              ...
              <TableCell>
                <div
                  onClick={() => handlePurchase(company._id)}
              ...
              <TableCell>
                <div
                  onClick={() => handleSales(company._id)}
                  style={{ cursor: "pointer" }}
                >
                  {company.salesCount}
                </div>
              ...
              <TableCell>
                <div
                  onClick={() => handleSupplier(company._id)}
                  style={{ cursor: "pointer" }}
                >
